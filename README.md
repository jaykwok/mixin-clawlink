...
对应环境项为 `MIXIN_AGY_MODEL` 与 `MIXIN_AGY_EFFORT`。`AGY_EFFORT` 只接受 `low`、`medium`、`high` 或空值，并在下条消息启动的 `agy --print` 进程中作为 `--effort` 传入。`/status` 会显示当前 agy 模型与推理强度。

`agy models` 可能先输出 `Fetching available models...`，随后以“稳定 slug + 显示名”逐行列出模型；连接器会过滤进度提示，只把 slug 保存到 `AGY_MODEL`。agy 1.1.5 的 `--mode` 只接受 `accept-edits`、`plan`，配置为 `default` 或留空时连接器不会传 `--mode`。

agy 1.1.5 还允许在自定义 agent frontmatter 中写 `model: flash`、`model: pro` 或 `model: inherit`。本连接器通过 `AGY_AGENT`/`--agent` 调用该 agent，frontmatter 由 agy 自己解析；若希望完全采用 agent 的模型层级，建议将 `AGY_MODEL` 与 `AGY_EFFORT` 留空，避免会话级参数覆盖 agent 默认值。

`CLAUDE_PERMISSION` 默认使用 `auto`：Agent 自行判断，必要时申请权限；`CLAUDE_DANGER_CONFIRM=1` 时，命中危险模式的 Bash 仍会强制在量子密信中确认。

`AGY_PERMISSION` 控制 agy 的工具权限：`bypass`（默认）= `--dangerously-skip-permissions` 全自动放行；`settings` = 不传该 flag，改由 agy 的 `settings.json`（`toolPermission`/`sandbox`/`trustedWorkspaces`）控制，更精细安全。本项目统一要求 agy ≥ 1.1.5；启动时若探测到旧版本会直接提示升级。
