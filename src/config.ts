...
export interface Cfg {
  ...
  /** agy 权限策略：bypass=全自动；settings=使用 settings.json 细粒度权限。 */
  AGY_PERMISSION: "bypass" | "settings";
  /** 是否启用 agy 终端 sandbox。 */
  AGY_SANDBOX: boolean;
  /** agy --print-timeout 秒数。 */
  AGY_TIMEOUT_S: number;
...
function apply(): void {
  ...
  // --mode 只接受 accept-edits/plan；default 等价于不传 flag。
  ...
  cfg.AGY_SANDBOX = ["1", "true", "yes", "on"].includes(envStr("MIXIN_AGY_SANDBOX", "0").trim().toLowerCase());
  const agyTimeout = parseInt(envStr("MIXIN_AGY_TIMEOUT_S", "600"), 10);
  cfg.AGY_TIMEOUT_S = Number.isFinite(agyTimeout) && agyTimeout > 0 ? agyTimeout : 600;
}
...
export const EDITABLE: EditableEntry[] = [
  ...
  { key: "FILE_RETURN_INSTRUCTION", env: "MIXIN_FILE_RETURN_INSTRUCTION", kind: "str", desc: "Claude 回传文件约定提示词（agy 使用 JSON Schema）" },
  ...
  { key: "AGY_MODEL", env: "MIXIN_AGY_MODEL", kind: "str", allowEmpty: true, desc: "agy 稳定模型 slug（留空用默认；1.1.10+）" },
  { key: "AGY_EFFORT", env: "MIXIN_AGY_EFFORT", kind: "choice", allowEmpty: true, choices: ["low", "medium", "high"], desc: "agy 推理强度（low/medium/high；留空由模型决定）" },
  ...
  { key: "AGY_PERMISSION", env: "MIXIN_AGY_PERMISSION", kind: "choice", choices: ["bypass", "settings"], desc: "agy 权限策略：bypass=全自动；settings=使用 settings.json 细粒度权限" },
  { key: "AGY_SANDBOX", env: "MIXIN_AGY_SANDBOX", kind: "bool", desc: "agy 是否启用终端 sandbox" },
  { key: "AGY_TIMEOUT_S", env: "MIXIN_AGY_TIMEOUT_S", kind: "int", desc: "agy 单轮 print 超时秒数" },
];
